...
async function cargarSelect(url, selectId, defaultText) {
    ...
    try {
        ...
        data.forEach(item => {
            // CORRECCIÓN CLAVE: El orden estricto de IDs para evitar colisiones
            const id = item.id || item.municipioid || item.departamentoid || item.nacionalidadid || item.generoid || item.estadocivilid || item.niveleducativoid || item.idiomaid || item.sectorid || item.tipoempresaid || item.estadoid || item.tipoempleoid || item.modalidadid || Object.values(item)[0];
            const nombre = item.nombre || item.descripcion || item.contexto || Object.values(item)[1];
            el.add(new Option(nombre, id));
        });
    } catch (error) { console.error("Error cargando select:", selectId); }
}

// ================= MODAL DE EDICIÓN E IMAGEN =================
...
// ================= 3. LÓGICA DE VACANTES Y REQUISITOS =================
function inicializarEventosVacantes() {
    ...
    // Enviar Formulario (Empleo -> Vacante -> Requisitos)
    document.getElementById('formNuevaVacante')?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Insertamos los requisitos seleccionados actualmente
...
window.editarVacante = async function(id) {
    ...
    try {
        ...
        // 3. Cargar Vacante (para sacar el Límite de Cantidad)
        try {
    ...
    try {
        ...
        lista.innerHTML = postulaciones.map(p => {
            ...
            if (p.candidatoid && typeof p.candidatoid === 'object') {
                ...
                // Si todo está vacío, mostramos el ID
            ...
            // 2. Extraer el estado (ej. "En Revisión")
...
// ================= 6. BORRADO LÓGICO DE CUENTA =================
function inicializarBorradoCuenta() {
    const btnEliminar = document.getElementById('btnEliminarCuenta');
    if (btnEliminar) {
        btnEliminar.addEventListener('click', async () => {
            const confirmacion = confirm("⚠️ ADVERTENCIA CRÍTICA: Estás a punto de desactivar el perfil de tu empresa.\n\nYa no podrás gestionar tus vacantes publicadas ni revisar nuevas postulaciones.\n\n¿Estás completamente seguro de continuar?");
            ...
            if (confirmacion) {
                ...
                try {
                    ...
                    if (res.ok) {
                        alert("La cuenta de la empresa ha sido desactivada exitosamente.");
